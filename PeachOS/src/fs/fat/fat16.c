...
// fat directory entry attribute bit mask
...
struct fat_private{
    ...
    struct disk_stream* cluster_read_stream; // stream data clusters
    struct disk_stream* fat_read_stream; // stream file allocation table
    struct disk_stream* directory_stream; // stream directory
};
...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector){
    ...
    int res = 0;
    int i = 0;
    int directory_start_pos = fat_private->root_directory.sector_pos * disk->sector_size;
    ...
    while(1){
        ...
        if(item.filename[0] == 0x00){ // are we done
            break; 
        }
        if(item.filename[0] == 0xE5){ // is item unused
            continue;
        }
...
int fat16_resolve(struct disk* disk){
    ...
    if(fat_private->header.shared.extended_header.signature != 0x29){ // if not fat16
        res = -EFSNOTUS;
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset){
    ...
    for(int i = 0; i < clusters_ahead; i++){
        ...
        if(entry == 0xFF8 || entry == 0xFFF){ //last entry in file
            res = -EIO;
            goto out;
        }

        if(entry  == PEACHOS_FAT16_BAD_SECTOR){ // if sector is bad (corrupted)
            res = -EIO;
            goto out;
        }
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk, struct fat_directory* directory, const char* name){
    ...
    for(int i = 0; i < directory->total; i++){
        ...
        if(istrncmp(tmp_filename, name, sizeof(tmp_filename)) == 0) //found it, create new fat item
